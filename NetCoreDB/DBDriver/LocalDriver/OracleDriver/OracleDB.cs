...
namespace NetCoreDBModule.NetCoreDB.DBDriver.LocalDriver.OracleDriver
{
  public  class OracleDB : IDBDriver
    {
        ...
        ConnectPool.ConnectionPoolManager.ConnectionPool connectionPool = new ConnectPool.ConnectionPoolManager.ConnectionPool(conn_str, ConnectionType.Odbc, 2, 1);
        ...
        public OracleDB(string constr, string key)
        {
            ...
            if (connectionPool == null)
            {
                connectionPool = new ConnectPool.ConnectionPoolManager.ConnectionPool(constr, ConnectionType.Odbc, 2, 1);
                connectionPool.StartServices();
                if (!string.IsNullOrEmpty(key))
                {
                    connection = connectionPool.GetConnectionFormPool(ApplkKeyUser, 0);
                ...
                {
                    connection = connectionPool.GetConnectionFormPool("steve2", 0);
                }
            }
        ...
        private DbConnection GetConstructDBConnect()
        {
            ...
            try
            {
                if (connection == null)
                {
                   connection = GetOracleConnectPool.getPool().getConnection();//后面新增
                }
                if (connection.State == ConnectionState.Closed)
                {
                    //connection = null;
        ...
        private DbConnection GetConstructDBConnect(string constr)
        {
            ...
            try
            {
                if (connection == null)
                {
                    connection = GetOracleConnectPool.getPool(constr).getConnection();//后面新增
                }
                if (connection.State == ConnectionState.Closed)
                {
                    //connection = null;
