...
namespace NetCoreDBModule.NetCoreDB.DBDriver.LocalDriver.ConnectPool.ConnectionPoolManager
{
    ...
    public class ConnectionPool : IDisposable
    {
        ...
        public ConnectionPool(string connectionString, ConnectionType connType, int maxConnection, int minConnection)
        {
        ...
        {
            return GetConnectionFormPool(key, ConnectionLevel.High);
        }
        ...
        public DbConnection GetConnectionFormPool(object key, ConnectionLevel connLevel)
        {
            lock (this)
            {
                ...
                {
                    ConnectionStruct cnn = (ConnectionStruct)hstUsingConn[key];
                    cnn.IsCanAllot = true;
                    cnn.IsRepeat = true;
                    //cnn.Enable = true;
                    return GetConnectionFormPool_Return(key, cnn, ConnectionLevel.High);
                }
        ...
        protected DbConnection GetConnectionFormPool_None(object key)
        {
            ...
            if (al.Count == 0)
                return GetConnectionFormPool_Return(key, null, ConnectionLevel.High);//发出异常
            else
                return GetConnectionFormPool_Return(key, (al[al.Count / 2]), ConnectionLevel.High);//返回连接
        }
        ...
        private DbConnection GetConnectionFormPool_Return(object key, ConnectionStruct connStruct, ConnectionLevel connLevel)
        {
            try
            {
                if (connStruct == null)
                {
                    ...
                    {
                        connStruct = al_All[0];
                       // hstUsingConn.Add("temp", al_All[al_All.Count-1]);
                    }
                    else
                    {
                        connStruct = CreateConnection(ConnectionString, ConnectionType.Odbc);
                ...
                //if (connLevel == ConnectionLevel.High)
                //{
                //    connStruct.IsCanAllot = false;
                //    connStruct.IsRepeat = false;
                //}
